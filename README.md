## Resources

[Training](https://www.udemy.com/course/sharepointframeworkspfxdevelopment/learn/lecture/19133576#overview)

[Download SharePoint Management Shell on Mac](https://chatgpt.com/share/e/677fe7de-bc30-8009-9238-bbee1bd9e3dd)

[Register an Entra ID Application to use with PnP PowerShell](https://pnp.github.io/powershell/articles/registerapplication.html)

[PnP PowerShell cmdlets](https://pnp.github.io/powershell/cmdlets/)

[Microsoft.Online.SharePoint.PowerShell](https://learn.microsoft.com/en-us/powershell/module/sharepoint-online/?view=sharepoint-ps)

[SharePoint Framework roadmap](https://learn.microsoft.com/en-us/sharepoint/dev/spfx/roadmap)

## Graph Explorer
[Graph Explorer](https://developer.microsoft.com/en-us/graph/graph-explorer)

## Samples by Framework
[Samples by Framework](https://aka.ms/spfx-webparts/)

## TypeScript Tutorial
[TypeScript](https://www.tutorialspoint.com/typescript/index.htm)

[Working with TypeScript in Visual Studio Code](https://www.youtube.com/watch?v=6x8Xhhe37WE)

## npm
[npm](https://www.npmjs.com/)

## nvm
[nvm](https://github.com/nvm-sh/nvm)

1. nvm list
2. nvm use 'node version'

## How to install Node Version Manager (NVM) for Windows 10?
[How to install Node Version Manager (NVM) for Windows 10?](https://dev.to/skaytech/how-to-install-node-version-manager-nvm-for-windows-10-4nbi)

## SharePoint Framework
[Overview of the SharePoint Framework](https://docs.microsoft.com/en-us/sharepoint/dev/spfx/sharepoint-framework-overview?WT.mc_id=m365-15744-cxa)

[Set up your SharePoint Framework development environment](https://docs.microsoft.com/en-us/sharepoint/dev/spfx/set-up-your-development-environment)

[Build your first SharePoint client-side web part (Hello World part 1)](https://docs.microsoft.com/en-us/sharepoint/dev/spfx/web-parts/get-started/build-a-hello-world-web-part)

[Build your first SharePoint Framework Extension (Hello World part 1)](https://docs.microsoft.com/en-us/sharepoint/dev/spfx/extensions/get-started/build-a-hello-world-extension)

[Develop web parts with the SharePoint Framework](https://docs.microsoft.com/en-us/learn/modules/sharepoint-spfx-web-parts/)

[PnP SPFx Yeoman generator](https://pnp.github.io/generator-spfx/)

[Lab - Getting Started with the SharePoint Framework](https://aka.ms/spdevwsM4L1)

[Lab - SharePoint Framework WebParts](https://github.com/SharePoint/sp-dev-training-spfx-web-parts#lab---developing-with-the-sharepoint-framework-web-parts)

[Lab - Working with the Web Part Property Pane](https://github.com/SharePoint/sp-dev-training-spfx-webpart-proppane/#lab---working-with-the-web-part-property-pane)

[Lab - Work with SharePoint Content using the SharePoint Framework](https://github.com/SharePoint/sp-dev-training-spfx-spcontent#lab---work-with-sharepoint-content-using-the-sharepoint-framework)

[Lab - Extend the SharePoint user interface with SharePoint Framework extensions](https://github.com/SharePoint/sp-dev-training-spfx-extensions#lab---extend-the-sharepoint-user-interface-with-sharepoint-framework-extensions)

[Lab - Leverage Microsoft Graph & third-party APIs](https://github.com/SharePoint/sp-dev-training-spfx-graph-3rdpartyapis#lab---leverage-microsoft-graph--third-party-apis)

[Lab - Using React and Office UI Fabric React Components](https://github.com/SharePoint/sp-dev-training-spfx-react-fabric#lab---using-react-and-office-ui-fabric-react-components)

[Lab - Deploy SharePoint Framework components to production](https://github.com/SharePoint/sp-dev-training-spfx-deployment#lab---deploy-sharepoint-framework-components-to-production)

[Lab - Build Microsoft Teams customization using the SharePoint Framework](https://github.com/SharePoint/sp-dev-training-spfx-teams-dev#lab---build-microsoft-teams-customization-using-the-sharepoint-framework)

## PnP
[PnP Microsoft 365](https://pnp.github.io/)

[Reusable property pane controls for the SharePoint Framework solutions](https://pnp.github.io/sp-dev-fx-property-controls/)

## CDNs
[Use the Office 365 Content Delivery Network (CDN) with SharePoint Online](https://docs.microsoft.com/en-us/microsoft-365/enterprise/use-microsoft-365-cdn-with-spo?view=o365-worldwide)

## Switch NodeJS Versions on Machine
[NVM](https://github.com/nvm-sh/nvm)

[Gulp Packages](https://www.docker.com/resources/what-container)

[nvm-windows](https://github.com/coreybutler/nvm-windows/releases)

[How to install Node Version Manager (NVM) for Windows 10?](https://dev.to/skaytech/how-to-install-node-version-manager-nvm-for-windows-10-4nbi)

[SPFX Development environment trouble, has anyone seen lookups.flatmap error?](https://stackoverflow.com/questions/67231581/spfx-development-environment-trouble-has-anyone-seen-lookups-flatmap-error)

## Fluent UI
[Fluent UI](https://developer.microsoft.com/en-us/fluentui#/)

## SharePoint Site Design
[Site Designer](https://www.sitedesigner.io)

[SharePoint Look Book](https://adoption.microsoft.com/en-us/sharepoint-look-book/)

## Fluent UI Icons
[Fluent UI Icons](https://developer.microsoft.com/en-us/fluentui#/styles/web/icons)

## HillBilly Tabs

[HillBilly Tabs](https://www.youtube.com/watch?v=jR62s32iycg)

[HillBilly Tabs Repo](https://github.com/mrackley/HillbillyTabs-4.0)

[Building Applications and Solutions with Microsoft 365 Core Services (MS-600T00)](https://www.globalknowledge.com/us-en/course/184331/building-applications-and-solutions-with-microsoft-365-core-services-ms-600t00/offerings/)

## jQuery Promises
[Working with jQuery’s AJAX, Promises and Deferred objects](http://michaelsoriano.com/working-with-jquerys-ajax-promises-and-deferred-objects)

## PnP
[GitHub PnP](https://github.com/pnp/pnp)

[PnPjs](https://pnp.github.io/pnpjs/pnpjs/)

[PnP SPFx](https://pnp.github.io/sp-dev-fx-property-controls/controls/PropertyFieldPeoplePicker/)

## Fluent UI Web
[Fluent UI Web](https://github.com/microsoft/fluentui/blob/master/README.md)

## Fluent UI
[Fluent UI](https://developer.microsoft.com/en-us/fluentui#/)

## API Docs
[API Docs](https://docs.microsoft.com/en-us/javascript/api/sp-property-pane/ipropertypanecustomfieldprops?view=sp-typescript-latest)

## Refs
[BaseApplicationCustomizer class](https://docs.microsoft.com/en-us/javascript/api/sp-application-base/baseapplicationcustomizer?view=sp-typescript-latest)

## SharePoint online spfx script editor web part
[SharePoint online spfx script editor web part](https://www.enjoysharepoint.com/spfx-script-editor-web-part/)

## Lab
[Microsoft Campus](https://mslearningcampus.com/ClassEnrollment/2422974)

## Sharepoint Online: Integrate Power apps canvas form with SPFx
[Sharepoint Online: Integrate Power apps canvas form with SPFx](https://social.technet.microsoft.com/wiki/contents/articles/52935.sharepoint-online-integrate-power-apps-canvas-form-with-spfx.aspx)

## Use Office UI Fabric React components in your SharePoint client-side web part
[Use Office UI Fabric React components in your SharePoint client-side web part](https://docs.microsoft.com/en-us/sharepoint/dev/spfx/web-parts/get-started/use-fabric-react-components)

## Introducing SharePoint Typed Item - Visual Studio Code extension
[Introducing SharePoint Typed Item - Visual Studio Code extension](https://spblog.net/post/2019/02/28/introducing-sharepoint-typed-item-visual-studio-code-extension)

## Unable get Lookup column value using REST
[Unable get Lookup column value using REST](https://social.msdn.microsoft.com/Forums/en-US/6882078b-8e5e-4301-96cc-fe717b98a74f/unable-get-lookup-column-value-using-rest?forum=sharepointdevelopment)

## Rest Query-The attempted operation is prohibited because it exceeds the list view threshold enforced by the administrator.
[Rest Query-The attempted operation is prohibited because it exceeds the list view threshold enforced by the administrator.](https://social.msdn.microsoft.com/Forums/office/en-US/6e84eb21-37a6-42a5-9702-72bd44db44d7/rest-querythe-attempted-operation-is-prohibited-because-it-exceeds-the-list-view-threshold-enforced?forum=sharepointdevelopment)

## Search Webpart App - GitHub
[pnp/sp-dev-fx-webparts](https://github.com/pnp/sp-dev-fx-webparts/tree/main/samples/react-directory)

## Serch App - Affirma Labs
[Sandbox](https://affirmalab.sharepoint.com/sites/ContosoHealthcare/SitePages/Test-Staff-Directory.aspx)

## Preview on upcoming list extensibility options with SPFx v1.15
[Preview on upcoming list extensibility options with SPFx v1.15](https://www.youtube.com/watch?v=90DWB9hjo-k)

## Ins and Outs of Viva Connect Dashboard - Part 1
[Ins and Outs of Viva Connect Dashboard - Part 1](https://www.youtube.com/watch?v=x_XZx3DUF38&t=761s)

## Execute Power Automate Workflow from SPFx
[Execute Power Automate Workflow from SPFx](https://www.c-sharpcorner.com/article/execute-power-automate-workflow-from-spfx/)

## @microsoft/sp-webpart-base package
[@microsoft/sp-webpart-base package](https://docs.microsoft.com/en-us/javascript/api/sp-webpart-base?view=sp-typescript-latest)

## @microsoft/sp-http
[@microsoft/sp-http](https://www.npmjs.com/package/@microsoft/sp-http)

## Using Office UI Fabric Core and Fabric React in SharePoint Framework
[Using Office UI Fabric Core and Fabric React in SharePoint Framework](https://docs.microsoft.com/en-us/sharepoint/dev/spfx/office-ui-fabric-integration)

## Instafeed.js
[Instafeed.js](https://instafeedjs.com/#/)

## 'node' is not recognized as an internal or external command, operable program or batch file ❓ [How to Solve]
['node' is not recognized as an internal or external command, operable program or batch file ❓ [How to Solve]](https://love2dev.com/blog/node-is-not-recognized-as-an-internal-or-external-command/)

## SharePoint Framework: Retrieve and Display SharePoint List Items using REST API and React JS
[SharePoint Framework: Retrieve and Display SharePoint List Items using REST API and React JS](https://social.technet.microsoft.com/wiki/contents/articles/37948.sharepoint-framework-retrieve-and-display-sharepoint-list-items-using-rest-api-and-react-js.aspx)

## Retrieve SharePoint List Items Using SharePoint Framework Development Model
[Retrieve SharePoint List Items Using SharePoint Framework Development Model](https://www.c-sharpcorner.com/article/retrieve-sharepoint-list-items-using-sharepoint-framework-development-model/)

## Microsoft Graph Toolkit: UI Components and Authentication Providers for Microsoft Graph
[Microsoft Graph Toolkit: UI Components and Authentication Providers for Microsoft Graph](https://mgt.dev/?path=/story/overview--page)

## Building personal events web part with SPFx and Microsoft Graph Toolkit
[Building personal events web part with SPFx and Microsoft Graph Toolkit](https://www.youtube.com/watch?v=St9QvaK_HUU)

## Calendar Feed
[Calendar Feed](https://github.com/pnp/sp-dev-fx-webparts/tree/main/samples/react-calendar-feed)

## Reusable React controls for your SharePoint Framework solutions
[@pnp/spfx-controls-react](https://pnp.github.io/sp-dev-fx-controls-react/)

## office-ui-fabric-react package
[office-ui-fabric-react package](https://docs.microsoft.com/en-us/javascript/api/office-ui-fabric-react?view=office-ui-fabric-react-latest)
=======
## Errors
[Fix Onedrive The tag present in the reparse point buffer is invalid](https://www.itexperience.net/fix-onedrive-the-tag-present-in-the-reparse-point-buffer-is-invalid/)

## Create Event - Microsoft Graph
[Create Event](https://docs.microsoft.com/en-us/graph/api/user-post-events?view=graph-rest-1.0&tabs=http#request-body)

## Microsoft Graph API - Schedule repeating appointments as recurring events in Outlook
[Schedule repeating appointments as recurring events in Outlook](https://docs.microsoft.com/en-us/graph/outlook-schedule-recurring-events)

## Graph Explorer
[Graph Explorer](https://developer.microsoft.com/en-us/graph/graph-explorer)

## SharePoint Framework Node Version
14.15.0 (Currently using 64-bit executable)

## Command Line Actions

### Identify and Kill Process
lsof -i :4321

kill -9 '<PID>'

gulp trust-dev-cert

gulp build 

gulp serve

gulp bundle --ship

gulp package-solution --ship

yo @microsoft/sharepoint

yo @microsoft/sharepoint --skip install

## Microsoft Certifications

[Microsoft 365 Certified: Fundamentals](https://learn.microsoft.com/en-us/credentials/certifications/microsoft-365-fundamentals/?practice-assessment-type=certification)

[Microsoft 365 Certified: Teams Administrator Associate](https://learn.microsoft.com/en-us/credentials/certifications/m365-teams-administrator-associate/?practice-assessment-type=certification)

[Microsoft Certified: Power Platform Functional Consultant Associate](https://learn.microsoft.com/en-us/credentials/certifications/power-platform-functional-consultant-associate/?practice-assessment-type=certification)

[Microsoft Certified: Power Platform Fundamentals](https://learn.microsoft.com/en-us/credentials/certifications/power-platform-fundamentals/?practice-assessment-type=certification)

[Microsoft Power Platform Developer](https://learn.microsoft.com/en-us/training/courses/pl-400t00)

## Office 365 Service End Points
[IP address configuration](https://learn.microsoft.com/en-us/power-automate/ip-address-configuration#required-services)

## Compliance
[Trust Center](https://www.microsoft.com/en-us/trust-center)

## Dataverse
[Explore Microsoft Dataverse](https://learn.microsoft.com/en-us/training/modules/identify-foundational-components-microsoft-power-platfor/4-explore-microsoft-dataverse)

## Microsoft Syntex
[Microsoft Syntex](https://support.microsoft.com/en-us/office/microsoft-syntex-59476568-0a30-4a3e-8a12-206b8ae47a00)

## Individual
https://graph.microsoft.com/v1.0/me/calendars/AAMkAGE1OWMyZDUzLWFjNWMtNGZkMy1iMzYxLWQ0MTc1ZGRhNjEzNgBGAAAAAABWF_h4C6aqR5NoaI_tyHYRBwAEy5hfEG9qSYTA69r4K9ZSAAAAAAEGAAAEy5hfEG9qSYTA69r4K9ZSAAAMSh7SAAA=/calendarPermissions


Can View When Im Busy'

{ "emailAddress": { "address": "testUser@budoapps2.onmicrosoft.com" }, "role": "freeBusyRead" }


Can View Titles and Locations'

{ "emailAddress": { "address": "testUser@budoapps2.onmicrosoft.com" }, "role": "limitedRead" }

'Can View All Details'

{ "emailAddress": { "address": "testUser@budoapps2.onmicrosoft.com" }, "role": "read" }

'Can Edit'

{ "emailAddress": { "address": "fernando.l.narvaez.ctr@mail.mil" }, "role": "write" }


## Migration
[Moving Lists, Forms & Flows Between Environments](https://chatgpt.com/share/e/67b6379e-1450-8009-9673-5117c34b6


## CoPilot Studio
[CoPilot Studio](https://copilot.microsoft.com/)


## Dataverse Licensing
[Dataverse Licensing](https://chatgpt.com/share/e/67bf4ed6-9628-8009-ac46-264ea67aeda8)


## Flow Errors
[Best practices when updating a flow used by a Power App](https://learn.microsoft.com/en-us/troubleshoot/power-platform/power-apps/connections/best-practices-when-updating-a-flow)

## DevOps
[DevOps](https://dev.azure.com/lewisshaun)


MissionDashboardV3_gallery OPS Mission Tracker



Search(

If(And(ScreenSelCtrls_FilterMissionStatus.Selected.Result = "Status - All", ScreenCtrls_FilterMissionType.Selected.Result = "Mission Type - All"),
Switch(ScreenSelCtrls_ArchiveMission.Checked,
true,Switch(varDisplaySort,
    true,Sort(colMissionListV3,EndDate,SortOrder.Descending),
    false,Sort(colMissionListV3,StartDate,SortOrder.Ascending)
    ),
false,Switch(varDisplaySort,
    true,Sort(Filter(colMissionListV3,Not(Archive)),EndDate,SortOrder.Descending),
    false,Sort(Filter(colMissionListV3,Not(Archive)),StartDate,SortOrder.Ascending)
)),

And(ScreenSelCtrls_FilterMissionStatus.Selected.Result = "Status - All", ScreenCtrls_FilterMissionType.Selected.Result <> "Mission Type - All"),
Filter(Switch(ScreenSelCtrls_ArchiveMission.Checked,
true,Switch(varDisplaySort,
    true,Sort(colMissionListV3,EndDate,SortOrder.Descending),
    false,Sort(colMissionListV3,StartDate,SortOrder.Ascending)
    ),
false,Switch(varDisplaySort,
    true,Sort(Filter(colMissionListV3,Not(Archive)),EndDate,SortOrder.Descending),
    false,Sort(Filter(colMissionListV3,Not(Archive)),StartDate,SortOrder.Ascending)
)),MissionType = ScreenCtrls_FilterMissionType.Selected.Result),

And(ScreenSelCtrls_FilterMissionStatus.Selected.Result <> "Status - All", ScreenCtrls_FilterMissionType.Selected.Result = "Mission Type - All"),
Filter(Switch(ScreenSelCtrls_ArchiveMission.Checked,
true,Switch(varDisplaySort,
    true,Sort(colMissionListV3,EndDate,SortOrder.Descending),
    false,Sort(colMissionListV3,StartDate,SortOrder.Ascending)
    ),
false,Switch(varDisplaySort,
    true,Sort(Filter(colMissionListV3,Not(Archive)),EndDate,SortOrder.Descending),
    false,Sort(Filter(colMissionListV3,Not(Archive)),StartDate,SortOrder.Ascending)
)),Status = ScreenSelCtrls_FilterMissionStatus.Selected.Result),

Filter(Switch(ScreenSelCtrls_ArchiveMission.Checked,
true,Switch(varDisplaySort,
    true,Sort(colMissionListV3,EndDate,SortOrder.Descending),
    false,Sort(colMissionListV3,StartDate,SortOrder.Ascending)
    ),
false,Switch(varDisplaySort,
    true,Sort(Filter(colMissionListV3,Not(Archive)),EndDate,SortOrder.Descending),
    false,Sort(Filter(colMissionListV3,Not(Archive)),StartDate,SortOrder.Ascending)
)),And(Status = ScreenSelCtrls_FilterMissionStatus.Selected.Result,MissionType = ScreenCtrls_FilterMissionType.Selected.Result))
)

,ScreenCtrls_SearchText.Value,Location,MissionID,MissionTitle)





-----------------------------------------
// Archive toggle logic
With(
    {
        baseList: If(
            ScreenSelCtrls_ArchiveMission.Checked,
            colMissionListV3,
            Filter(colMissionListV3, Not(Archive))
        )
    },
    With(
        {
            sortedList: If(
                varDisplaySort,
                Sort(baseList, EndDate, SortOrder.Descending),
                Sort(baseList, StartDate, SortOrder.Ascending)
            )
        },
        With(
            {
                filteredList: Switch(
                    true,
                    And(
                        ScreenSelCtrls_FilterMissionStatus.Selected.Result = "Status - All",
                        ScreenCtrls_FilterMissionType.Selected.Result = "Mission Type - All"
                    ),
                    sortedList,

                    And(
                        ScreenSelCtrls_FilterMissionStatus.Selected.Result = "Status - All",
                        ScreenCtrls_FilterMissionType.Selected.Result <> "Mission Type - All"
                    ),
                    Filter(
                        sortedList,
                        MissionType = ScreenCtrls_FilterMissionType.Selected.Result
                    ),

                    And(
                        ScreenSelCtrls_FilterMissionStatus.Selected.Result <> "Status - All",
                        ScreenCtrls_FilterMissionType.Selected.Result = "Mission Type - All"
                    ),
                    Filter(
                        sortedList,
                        Status = ScreenSelCtrls_FilterMissionStatus.Selected.Result
                    ),

                    // Default case: filter by both
                    Filter(
                        sortedList,
                        Status = ScreenSelCtrls_FilterMissionStatus.Selected.Result &&
                        MissionType = ScreenCtrls_FilterMissionType.Selected.Result
                    )
                )
            },
            // Final Search
            Search(
                filteredList,
                ScreenCtrls_SearchText.Value,
                "Location",
                "MissionID",
                "MissionTitle"
            )
        )
    )
)

---------

Switch(varReportView,
    "Active",Filter(colMissionListV3,And(JECC_Decision = "Concur",Archive = Text(false),StartDate <= Today(),EndDate >= Today(),CountRows(Filter(MissionPersonnel,percategory = "Approved"))>0)),
    "Future",Filter(colMissionListV3,And(JECC_Decision = "Concur",Archive = Text(false),StartDate > Today()))
)


-------------


Switch(
    varReportView,
    
    "Active",
    Filter(
        colMissionListV3,
        And(
            JECC_Decision = "Concur",
            Archive = Text(false),
            StartDate <= Date(2025, 3, 15),
            EndDate >= Date(2025, 3, 15),
            CountRows(Filter(MissionPersonnel, percategory = "Approved")) > 0
        )
    ),
    
    "Future",
    Filter(
        colMissionListV3,
        And(
            JECC_Decision = "Concur",
            Archive = Text(false),
            StartDate > Date(2025, 3, 15)
        )
    )
)

------------------

SortByColumns(AddColumns(Filter(ThisItem.MissionPersonnel,percategory = "Approved"),perstartdt_adj,If(IsBlank(perstartdt),ThisItem.StartDate,perstartdt),perenddt_adj,If(IsBlank(perenddt),ThisItem.EndDate,perenddt)),"perstartdt",SortOrder.Descending,"perenddt",SortOrder.Descending,"pername",SortOrder.Ascending)



-----------------

Resolved issues in two Power Automate flows (ACP_JECC_JPSE_OPS_Events and ACP-JECC_JPSE_OPS_EventsRemove) to ensure consistent execution during mission event creation and deletion.

Fixed flow logic errors to enable successful and reliable operation of both flows.

Added missing search filters for the ‘Notes’ and ‘MissionTask’ fields, allowing missions to be searchable by these attributes.

Refactored the mission dashboard’s listing and search functionality for improved performance, code simplicity, and long-term maintainability.




Collect(colMissionListV3,ForAll(ParseJSON(varAPIspolistJPSEmis).value As APIjson,
{
    MissionListID: Value(Text(APIjson.ID)),
    MissionTitle: Text(APIjson.Title),
    MissionID: Text(APIjson.DEPORD),
    StartDate: DateValue(Text(APIjson.StartDate)),
    EndDate: DateValue(Text(APIjson.EndDate)),
    Location: Text(APIjson.Location),
    MissionType: Text(APIjson.MissionType.Value),
    MissionPartner: Substitute(Substitute(Substitute(Text(APIjson.MissionParter),Char(10)," "),"""","'"),Char(9),""),
    OPR: Substitute(Substitute(Substitute(Text(APIjson.OPR),Char(10)," "),"""","'"),Char(9),""),
    Status: Text(APIjson.Status.Value),
    PaxRequest: Value(Text(APIjson.PaxRequest)),
    JTIMS: Text(APIjson.JTIMS),
    XorgLoa: Text(APIjson.XorgLoa),
    JECC_Decision: Text(APIjson.JECC_Decision.Value),
    JPSE_Recommend: Text(APIjson.JPSE_Recommend.Value),
    Archive: Text(APIjson.Archive),
    MissionTask: Substitute(Substitute(Substitute(Text(APIjson.MissionTask),Char(10)," "),"""","'"),Char(9),""),
    Notes: Substitute(Substitute(Substitute(Text(APIjson.Notes),Char(10)," "),"""","'"),Char(9),""),
    SupportRequests: Substitute(Substitute(Substitute(Text(APIjson.SupportRequests),Char(10)," "),"""","'"),Char(9),""),
    Funding: Substitute(Substitute(Substitute(Text(APIjson.Funding),Char(10)," "),"""","'"),Char(9),""),
    LeadComments: Substitute(Substitute(Substitute(Text(APIjson.LeadComments),Char(10)," "),"""","'"),Char(9),""),
    TeamLead: Substitute(Substitute(Substitute(Text(APIjson.TeamLead),Char(10)," "),"""","'"),Char(9),""),
    TeamDeputy: Substitute(Substitute(Substitute(Text(APIjson.TeamDeputy),Char(10)," "),"""","'"),Char(9),""),
    MissionCoordinator: Substitute(Substitute(Substitute(Text(APIjson.MissionCoordinator),Char(10)," "),"""","'"),Char(9),""),
    EngagementOfficer: Substitute(Substitute(Substitute(Text(APIjson.EngagementOfficer),Char(10)," "),"""","'"),Char(9),""),
    DWG1: DateValue(Text(APIjson.OData__x0044_WG1)),
    DWG2: DateValue(Text(APIjson.OData__x0044_WG2)),
    POM: DateValue(Text(APIjson.POM)),
    Deploy: DateValue(Text(APIjson.Deploy)),
    Arrival: DateValue(Text(APIjson.Arrival)),
    RSOI: DateValue(Text(APIjson.RSOI)),
    RDWG1: DateValue(Text(APIjson.RDWG1)),
    RDWG2: DateValue(Text(APIjson.RDWG2)),
    Redeploy: DateValue(Text(APIjson.Redploy)),
    RPOM: DateValue(Text(APIjson.RPOM)),
    Arrival2: DateValue(Text(APIjson.Arrival2)),
    EarlyReturnDT: If(!IsBlankOrError(Text(APIjson.EarlyReturnDT)),DateValue(Text(APIjson.EarlyReturnDT))),
    MissionPersonnel: ForAll(ParseJSON(Text(APIjson.JSONpersonnel)) As ActionPer,
    {
        posntitle: Text(ActionPer.posntitle),
        perdodid: Text(ActionPer.perdodid,"0000000000"),
        pername: Substitute(Substitute(Substitute(Text(ActionPer.pername),Char(10)," "),"""","'"),Char(9),""),
        perfxgrp: Text(ActionPer.perfxgrp),
        permail: Text(ActionPer.permail),
        perstartdt: DateValue(Text(ActionPer.perstartdt)),
        perenddt: DateValue(Text(ActionPer.perenddt)),
        pernotes: Substitute(Substitute(Substitute(Text(ActionPer.pernotes),Char(10)," "),"""","'"),Char(9),""),
        percompo: Text(ActionPer.percompo),
        pergrd: Text(ActionPer.pergrd),
        permos: Text(ActionPer.permos),
        percategory: Text(ActionPer.percategory),
        pervolunteer: Text(ActionPer.pervolunteer)
    }),
    Modified: DateTimeValue(Text(APIjson.Modified)),
    ModifiedBy: Text(APIjson.Editor.Email)
}));



[{
    "posntitle": "",
    "perdodid": "1063773669",
    "pername": "ALBERT, MICHAEL S Col",
    "perfxgrp": " PLANS",
    "permail": "michael.s.albert2.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Florida JPG OPT Dep",
    "percompo": "USAFR",
    "pergrd": "O-6 ",
    "permos": "16G3",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1169765614",
    "pername": "ALEGADO , KATHERINE T MAJ",
    "perfxgrp": " PA",
    "permail": "",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Canada OPT 2 PA",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "46A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1283917653",
    "pername": "ARDEN, CHARLES Maj",
    "perfxgrp": "JPSE | IS",
    "permail": "charles.d.arden.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "FLANG OPT 2 Deputy",
    "percompo": "USAF",
    "pergrd": "O-4 ",
    "permos": "14N4",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1092643340",
    "pername": "BALICE, NICHOLAS A",
    "perfxgrp": " PA",
    "permail": "nicholas.balice.civ@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Florida JPG OPT PA Mentor",
    "percompo": "CIV",
    "pergrd": "GS-14",
    "permos": "1035",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1234612715",
    "pername": "BOETTCHER, RICHARD A LCDR",
    "perfxgrp": " SUST",
    "permail": "richard.a.boettcher.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "FLANG OPT 1",
    "percompo": "USNR",
    "pergrd": "O-4 ",
    "permos": "1445",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1163423916",
    "pername": "BOOTHBY, WILBUR F LTC",
    "perfxgrp": " SUST",
    "permail": "wilbur.f.boothby.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Canada OPT 1 Lead",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "70H",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1096796575",
    "pername": "BUXMAN, NICOLA M COL",
    "perfxgrp": " SUST",
    "permail": "nicola.m.buxman.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Canada OPT 1",
    "percompo": "USAR",
    "pergrd": "O-6 ",
    "permos": "12A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1040302006",
    "pername": "CAHILL, RALPH CAPT",
    "perfxgrp": "JECC | CMD GRP",
    "permail": "",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "FLANG OPT 1",
    "percompo": "USNR",
    "pergrd": "O-6 ",
    "permos": "1310",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1266140866",
    "pername": "CARLSON, BRYAN J LCDR",
    "perfxgrp": " OPS",
    "permail": "bryan.j.carlson4.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Canada OPT 2 Deputy",
    "percompo": "USNR",
    "pergrd": "O-4 ",
    "permos": "1125",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1283333686",
    "pername": "CARTER, ABIGAIL G MAJ",
    "perfxgrp": " PLANS",
    "permail": "abigail.g.carter2.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Florida Exercise Lead",
    "percompo": "USA",
    "pergrd": "O-4 ",
    "permos": "14A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1078076050",
    "pername": "CASTLE, ANTHONY CDR",
    "perfxgrp": " SUST",
    "permail": "",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "FLANG OPT 1 Deputy",
    "percompo": "USNR",
    "pergrd": "O-5 ",
    "permos": "1125",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1244436265",
    "pername": "CHU, KAR C MAJ",
    "perfxgrp": "JPSE | OPS",
    "permail": "",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Canada OPT 2",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "13A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1175849935",
    "pername": "EARLY, EDWARD LCDR",
    "perfxgrp": " PA",
    "permail": "edward.a.early.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "FLANG OPT 2 PA",
    "percompo": "USN",
    "pergrd": "O-4 ",
    "permos": "1650",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1060970021",
    "pername": "FEITH, RIES CDR",
    "perfxgrp": " SUST",
    "permail": "ries.j.feith.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "Florida JPG HQ",
    "percompo": "USNR",
    "pergrd": "O-5 ",
    "permos": "3100",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1254896663",
    "pername": "FISHER, JOSIAH L LTC",
    "perfxgrp": " PA",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG PA",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "46A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1198257393",
    "pername": "FOUNTAINE, TIMOTHY J LTC",
    "perfxgrp": " SUST",
    "permail": "timothy.j.fountaine.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 2",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "38F",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1017658022",
    "pername": "GEORGE, SCOTT LTC",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "30A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1300640245",
    "pername": "HALL, MATTHEW D MAJ",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "38A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1270100305",
    "pername": "HARDING, JOHN J Lt Col",
    "perfxgrp": " SUST",
    "permail": "john.j.harding.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG",
    "percompo": "USAF",
    "pergrd": "O-5 ",
    "permos": "21R",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1299413884",
    "pername": "JENNINGS, DOMINIQUE C LCDR",
    "perfxgrp": " SUST",
    "permail": "dominique.c.jennings.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 1 Lead",
    "percompo": "USNR",
    "pergrd": "O-4 ",
    "permos": "1110",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1408651448",
    "pername": "JOEL, ROSALIE MAJ",
    "perfxgrp": " PLANS",
    "permail": "rosalie.a.joel.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada Mentor",
    "percompo": "USA",
    "pergrd": "O-4 ",
    "permos": "59A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1046039196",
    "pername": "JONES, JEFFREY M",
    "perfxgrp": " PA",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG PA",
    "percompo": "CIV",
    "pergrd": "GS-14",
    "permos": "1035",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "0112254476",
    "pername": "KOBOLD, ROBERT MAJ",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada Mentor",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "JPSE",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1234567899",
    "pername": "LUCERO, DANIELLE Capt",
    "perfxgrp": "JPSE | PA",
    "permail": "danielle.d.lucero.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG HQ",
    "percompo": "USAF",
    "pergrd": "O-3 ",
    "permos": "35P4",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1268575550",
    "pername": "LYNCH, ADAM D LtCol",
    "perfxgrp": " OPS",
    "permail": "adam.d.lynch.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG HQ",
    "percompo": "USMCR",
    "pergrd": "O-5 ",
    "permos": "123",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1239372100",
    "pername": "MARSH, DANIEL H LTC",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "01A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1017389641",
    "pername": "MCDONALD, MICHAEL J Maj",
    "perfxgrp": " PLANS",
    "permail": "michael.j.mcdonald12.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG HQ",
    "percompo": "USAF",
    "pergrd": "O-4 ",
    "permos": "11M",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1142553441",
    "pername": "MCGRUE, CHARLES R LTC",
    "perfxgrp": " OPS",
    "permail": "charles.r.mcgrue.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada OPT 2 Lead",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "25A, 26A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1013494777",
    "pername": "MICHALSKI, CHRISTOPHER CDR",
    "perfxgrp": " IS",
    "permail": "christopher.m.michalski.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 1",
    "percompo": "USN",
    "pergrd": "O-5 ",
    "permos": "1830",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1013684550",
    "pername": "MINTON, ROY H LTC",
    "perfxgrp": "JPSE | PLANS",
    "permail": "roy.h.minton.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "38A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1045026643",
    "pername": "MORTON, GREGORY CDR",
    "perfxgrp": " IS",
    "permail": "gregory.s.morton.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG IS White Cell",
    "percompo": "USNR",
    "pergrd": "O-5 ",
    "permos": "1830",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1087616378",
    "pername": "NAU, CHRISTOPHER G LTC",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada OPT 1",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "25A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1058753978",
    "pername": "NELSON, ADRIAN LTC",
    "perfxgrp": " SUST",
    "permail": "adrian.l.nelson3.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG HQ Deputy",
    "percompo": "USA",
    "pergrd": "O-5 ",
    "permos": "90A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1246229763",
    "pername": "PETERS, MICHAEL CDR",
    "perfxgrp": " IS",
    "permail": "michael.l.peters1.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 2 Lead",
    "percompo": "USNR",
    "pergrd": "O-5 ",
    "permos": "1830",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1173454305",
    "pername": "POWERS, KEVIN CDR",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 2",
    "percompo": "USNR",
    "pergrd": "O-5 ",
    "permos": "0000",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1028829317",
    "pername": "RHEA, HARRY M LT",
    "perfxgrp": " IS",
    "permail": "harry.m.rhea.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG",
    "percompo": "USNR",
    "pergrd": "O-3 ",
    "permos": "123",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1262180740",
    "pername": "RHODA, BENJAMIN Lt Col",
    "perfxgrp": " SUST",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG Lead",
    "percompo": "USAFR",
    "pergrd": "O-5 ",
    "permos": "1310",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1393619429",
    "pername": "ROBERTSON, ERIK MAJ",
    "perfxgrp": " SUST",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada OPT 1 Deputy",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "123",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1293039271",
    "pername": "SALAZAR, JOSEPH A MAJ",
    "perfxgrp": "JPSE | SUST",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 1",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "90A",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1296659220",
    "pername": "VARNER, MARY A LTC",
    "perfxgrp": "JPSE | OPS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada OPT 2",
    "percompo": "USAR",
    "pergrd": "O-5 ",
    "permos": "UNK",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1088279987",
    "pername": "WALLICK, JOSHUA CDR",
    "perfxgrp": " PLANS",
    "permail": "joshua.l.wallick.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "FLANG OPT 2",
    "percompo": "USNR",
    "pergrd": "O-5 ",
    "permos": "123",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1239670519",
    "pername": "WILSON, ROBERT CAPT",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Canada OPT 1",
    "percompo": "USNR",
    "pergrd": "O-6 ",
    "permos": "0000",
    "percategory": "Approved",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1007248110",
    "pername": "CRAIN, BRADFORD CAPT",
    "perfxgrp": "JPSE | KM",
    "permail": "bradford.p.crain.mil@mail.mil",
    "perstartdt": "3/18/2025",
    "perenddt": "",
    "pernotes": "FLANG Mentor",
    "percompo": "USN",
    "pergrd": "O-6 ",
    "permos": "1310",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1250717349",
    "pername": "CHAVEZ, CHRIS J MAJ",
    "perfxgrp": "JPSE | PLANS",
    "permail": "christopher.j.chavez1.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "",
    "percompo": "USA",
    "pergrd": "O-4 ",
    "permos": "59A",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1293537689",
    "pername": "COX, NICHOLAS A GySgt",
    "perfxgrp": " SUST",
    "permail": "nicholas.a.cox32.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "removed",
    "percompo": "USMCR",
    "pergrd": "E-7 ",
    "permos": "1371",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1076721927",
    "pername": "FRITZ, JOHN CAPT",
    "perfxgrp": "JPSE | PLANS",
    "permail": "john.e.fritz.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG (T)",
    "percompo": "USN",
    "pergrd": "O-6 ",
    "permos": "1110",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1078818910",
    "pername": "FRYMAN, KEVIN S MAJ",
    "perfxgrp": " PLANS",
    "permail": "",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Removed",
    "percompo": "USAR",
    "pergrd": "O-4 ",
    "permos": "11A",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1175716820",
    "pername": "GRANDIN, JOHN Y CPO",
    "perfxgrp": "JPSE | PA",
    "permail": "john.y.grandin.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Removed",
    "percompo": "USN",
    "pergrd": "E-7 ",
    "permos": "MC",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1275027355",
    "pername": "HARRELL, ROBERT CDR",
    "perfxgrp": " PLANS",
    "permail": "robert.a.harrell.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Removed",
    "percompo": "USN",
    "pergrd": "O-5 ",
    "permos": "1310",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1267689179",
    "pername": "MALLORY, WILLIAM B LCDR",
    "perfxgrp": "JPSE | IS",
    "permail": "william.b.mallory4.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "",
    "percompo": "USN",
    "pergrd": "O-4 ",
    "permos": "1830",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1283419181",
    "pername": "WAGNER, MICHAEL Maj",
    "perfxgrp": " PLANS",
    "permail": "michael.d.wagner54.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "ITP",
    "percompo": "USAF",
    "pergrd": "O-4 ",
    "permos": "17D",
    "percategory": "Nominee",
    "pervolunteer": "false"
},
{
    "posntitle": "",
    "perdodid": "1387508003",
    "pername": "WYNN, REBECCA L Lt Col",
    "perfxgrp": "JPSE | PLANS",
    "permail": "rebecca.l.wynn4.mil@mail.mil",
    "perstartdt": "",
    "perenddt": "",
    "pernotes": "Florida JPG HQ Mentor",
    "percompo": "USAF",
    "pergrd": "O-5 ",
    "permos": "13B",
    "percategory": "Nominee",
    "pervolunteer": "false"
}]
